...
- [x] Task: Mettre à jour les types TypeScript (`Conversation`) dans la Webapp pour inclure `accountEmail`.
- [x] Task: Adapter la logique de synchronisation dans `sync.js` (extension) et `google-drive.ts` (webapp) pour gérer ce nouveau champ.
- [x] Task: Vérifier la persistance dans `localStorage` et la bonne remontée vers Google Drive via les outils de développement.
...
- [x] Task: Implémenter la détection des pages d'erreur "404" ou "Lien non trouvé" dans le script de contenu de l'extension.
- [x] Task: Créer le composant UI de la modale d'aide (HTML/CSS injecté).
- [x] Task: Connecter la logique : si erreur détectée ET `accountEmail` connu pour cette URL -> Afficher la modale.
- [x] Task: Écrire des tests d'intégration simulant une erreur de page pour valider l'apparition de la modale.
